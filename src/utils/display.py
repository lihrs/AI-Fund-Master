...
def format_trading_output(result):
    """
    格式化交易输出结果，用于GUI显示
    """
    if not result:
    ...
    if "analyst_signals" in result and result["analyst_signals"]:
        ...
        for analyst, signals in result["analyst_signals"].items():
            output.append(f"分析师: {analyst}")
            if isinstance(signals, dict):
                for ticker, signal in signals.items():
                    output.append(f"  {ticker}: {signal}")
            else:
                output.append(f"  信号: {signals}")
            output.append("")
