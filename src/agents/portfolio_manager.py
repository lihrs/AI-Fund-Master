...
def portfolio_management_agent(state: AgentState, agent_id: str = "portfolio_manager"):
    ...
    for ticker in tickers:
        ...
        available_cash = portfolio.get('cash', 100000)  # 默认10万现金
    ...
    def create_default_portfolio_output():
        ...
        for ticker in tickers:
            ...
            if bullish_count > bearish_count:
                ...
                reasoning = f"经过{total_analysts}位分析师综合评估,{bullish_count}位看多.{bearish_count}位看空.{neutral_count}位中性.多数分析师认为当前价格{current_prices.get(ticker, 0):.2f}元具有投资价值,建议适量建仓.平均置信度{avg_confidence:.1f}%."
            elif bearish_count > bullish_count:
                action = "sell" if portfolio.get("positions", {}).get(ticker, {}).get("long", 0) > 0 else "hold"
                action_confidence = avg_confidence
                reasoning = f"经过{total_analysts}位分析师综合评估,{bearish_count}位看空.{bullish_count}位看多.{neutral_count}位中性.多数分析师对当前价格{current_prices.get(ticker, 0):.2f}元持谨慎态度,"
                if action == "sell":
                ...
                action = "hold"
                action_confidence = avg_confidence
                reasoning = f"经过{total_analysts}位分析师综合评估,{bullish_count}位看多.{bearish_count}位看空.{neutral_count}位中性.分析师意见相对分化,当前价格{current_prices.get(ticker, 0):.2f}元,建议维持现状待观察.平均置信度{avg_confidence:.1f}%."
