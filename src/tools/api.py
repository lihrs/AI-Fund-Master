        ...
        if market == 'CN':
            ...
            if df is not None and not df.empty:
                ...
                if prices and len(prices) > 0:
                    ...
                    print(f"✓ {ticker} 价格数据缓存成功: {len(prices)} 条")
                    return prices
                else:
                    print(f"✗ {ticker} 价格数据为空，不进行缓存")
                    return []
            ...
            try:
                ...
                if df is not None and not df.empty:
                    ...
                    if all(col in df.columns for col in required_columns):
                        ...
                        if prices and len(prices) > 0:
                            _cache.set_prices(cache_key, [p.model_dump() for p in prices])
                            print(f"✓ {ticker} 港股价格数据缓存成功: {len(prices)} 条")
                            return prices
                        else:
                            print(f"✗ {ticker} 港股价格数据为空，不进行缓存")
                            return []
                    else:
                        print(f"港股数据列不完整: {list(df.columns)}")
                else:
                    print(f"无港股价格数据: {symbol}")
            except Exception as e:
                print(f"港股数据获取失败: {e}")
            
        elif market == 'US':
            # 美股数据 - 可以尝试使用AKShare的美股接口
            symbol = _convert_us_symbol(ticker)
            try:
                # AKShare可能支持一些美股数据
                print(f"美股数据获取尚未完全实现: {symbol}")
                # 这里可以后续添加美股数据获取逻辑
            except Exception as e:
                print(f"美股数据获取失败: {e}")
...
def get_financial_metrics(
    ...
    try:
        ...
        if market == 'CN':
                ...
                if df_financial is not None and not df_financial.empty:
                    ...
                    for i in range(min(limit, len(date_columns))):
                        ...
                        metrics = FinancialMetrics(
                            ...
                            enterprise_value=None,  # 将通过计算模块补充
                            price_to_earnings_ratio=period_data.get('市盈率', 0),
                            price_to_book_ratio=period_data.get('市净率', 0),
                            price_to_sales_ratio=period_data.get('市销率', 0),
                            enterprise_value_to_ebitda_ratio=None,  # 计算补充
                            enterprise_value_to_revenue_ratio=None,  # 计算补充
                            free_cash_flow_yield=None,  # 计算补充
                            peg_ratio=None,  # 计算补充
                            gross_margin=period_data.get('销售毛利率', 0) / 100 if period_data.get('销售毛利率') else None,
                            operating_margin=None,  # 计算补充
                            ...
                            return_on_invested_capital=None,  # 计算补充
                            asset_turnover=period_data.get('总资产周转率', 0),
                            inventory_turnover=period_data.get('存货周转率', 0),
                            ...
                            current_ratio=period_data.get('流动比率', 0),
                            quick_ratio=None,  # 需要计算
                            cash_ratio=None,  # 需要计算
                            operating_cash_flow_ratio=None,  # 需要计算
                            debt_to_equity=period_data.get('产权比率', 0),
                            ...
                            earnings_per_share_growth=None,  # 需要计算
                            ...
                            earnings_per_share=period_data.get('基本每股收益', 0),
                            book_value_per_share=period_data.get('每股净资产', 0),
                            free_cash_flow_per_share=period_data.get('每股现金流量净额', 0),
                        )
                        ...
                        try:
                            ...
                            # 合并财务报表数据
                            metrics_dict.update(line_items_data)
                            
                            # 增强财务指标
                            enhanced_dict = financial_calculator.enhance_financial_metrics(metrics_dict)
                    ...
                    if financial_metrics and len(financial_metrics) > 0:
                        ...
                        print(f"✓ {ticker} 财务指标数据缓存成功: {len(financial_metrics)} 条")
                        return financial_metrics
                    else:
                        print(f"✗ {ticker} 财务指标数据为空，不进行缓存")
                        return []
                else:
                    print(f"无财务数据: {ticker}")
                    return []
            ...
            # 港股数据 - 后续补充实现
            print(f"HK market financial data for {ticker} not yet implemented")
            
        elif market == 'US':
            # 美股数据 - 后续补充实现
            print(f"US market financial data for {ticker} not yet implemented")
